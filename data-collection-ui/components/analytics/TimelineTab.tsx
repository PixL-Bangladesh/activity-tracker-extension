...
import type { EventWithScreenshots, NetworkData } from "./types";
...
// Lazy loaded event visualization component
const LazyEventVisualization: React.FC<{
  event: EventWithScreenshots;
  events: any[];
  recordingStartTime: number;
  showController: boolean;
}> = ({ event, events, recordingStartTime, showController }) => {
  const [ref, isVisible] = useIntersectionObserver();
  const [hasLoaded, setHasLoaded] = useState(false);

  // Once visible, mark as loaded and keep it loaded
  useEffect(() => {
  ...
  return (
    <div
      ref={ref}
      className="border rounded-md overflow-auto max-w-[1400px] min-h-[200px]"
    >
      {hasLoaded ? (
        <EventVisualization
          event={event}
...
const TimelineTab: React.FC<TimelineTabProps> = ({
  ...
  const timelineEvents = useMemo(() => {
    ...
    networkData.forEach((event) => {
      ...
      const url = (event.data as NetworkData).url || "";
          ...
          {filteredEvents.length > 0 ? (
            <div className="space-y-8">
              {filteredEvents.map((event, index) => (
                <div
                  key={event.id}
                  className="relative pb-8 border-l-2 border-border pl-6"
                >
                  {/* Timeline dot */}
                  <div
                    className={`absolute -left-[5px] h-3 w-3 rounded-full ${
                      event.source === "agent" ? "bg-chart-1" : "bg-chart-2"
                    }`}
                  />
                  ...
                  {/* Lazy loaded event visualization */}
                  <LazyEventVisualization
