    ...
    switch (event.type) {
      ...
      case EventType.IncrementalSnapshot: {
        ...
        analysis.eventCounts.incrementalSnapshot.bySource[sourceName] =
          (analysis.eventCounts.incrementalSnapshot.bySource[sourceName] || 0) +
          1;
...
export function getClickEvents(
  ...
  return events.filter((event) => {
    ...
    return (
      ...
      (incrementalEvent.data as mouseInteractionData).type ===
 ...
 */
export function getInputEvents(
  events: eventWithTime[]
): incrementalSnapshotEvent[] {
  return events.filter((event) => {
    if (event.type !== EventType.IncrementalSnapshot) return false;
 ...
 */
export function getPluginEventsByName(
  events: eventWithTime[],
  pluginName: string
): eventWithTime[] {
  return events.filter((event) => {
