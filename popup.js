document.addEventListener('DOMContentLoaded', function () {
  ...
  const eventsMatrix = [[]];
  recordScreenButton.addEventListener('click', function () {
    ...
    recordScreenButton.textContent = 'Stop Recording';
    recordScreenButton.classList.add('recording');

    // periodically download the events array
    setInterval(() => {
      chrome.runtime.sendMessage({ action: 'downloadEvents', events: eventsMatrix }, function (response) {
        console.log('Downloaded events:', response);
      });
    }, 5 * 60 * 1000);
  });
});
